...
// THIS IS A MODIFIED VERSION MADE BY "Suado Cowboy" INSPIRED BY Blackwonder's BHOP DUCK JUMP PLUGIN
...
float g_heightVelocity[MAXPLAYERS + 1];
bool g_bIsFalling[MAXPLAYERS + 1];
...
public void OnClientPutInServer(int client)
{
	...
	g_heightVelocity[client] = 0.0;
	g_bIsFalling[client] = false;
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon, int &subtype, int &cmdnum, int &tickcount, int &seed, int mouse[2])
{
	...
	if (!(entityFlags & FL_ONGROUND))
	{
		float velocity[3];
		GetEntPropVector(client, Prop_Data, "m_vecAbsVelocity", velocity);
		if (velocity[2] < 0.0)
			g_bIsFalling[client] = true;
		
		if (g_bIsFalling[client] && -velocity[2] > g_heightVelocity[client])
			g_heightVelocity[client] = -velocity[2];
	...
	{
		g_bInJumpRelease[client] = false;
		if (g_bIsFalling[client] && (entityFlags & FL_DUCKING))
		{
			g_bIsFalling[client] = false;

            if (g_heightVelocity[client] > 0.0 && (buttons & IN_JUMP))
            {
            	float velocity[3];
				GetEntPropVector(client, Prop_Data, "m_vecAbsVelocity", velocity);
            	...
            	if (gravity == 0.0)
            	{
            		gravity = GetConVarFloat(FindConVar("sv_gravity"));
            	}
            	
				velocity[2] = g_heightVelocity[client];
